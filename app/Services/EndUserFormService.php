...
class EndUserFormService
{
    ...
    public function getFormStructure(int $formVersionId, ?int $userId = null, ?int $languageId = null): array
    {
        ...
        // Load translations (fields + stages + sections + transitions + toStage translations)
        $formVersion->load([
            ...
            'stageTransitions.toStage.translations' => function ($q) use ($languageId) {
                $q->where('language_id', $languageId);
            },
        ]);
        ...
        // Get available transitions from initial stage (localized label + to stage name)
        $availableTransitions = $formVersion->stageTransitions()
            ->where('from_stage_id', $initialStage->id)
            ->with([
                'toStage',
                'toStage.translations' => function ($q) use ($languageId) {
                    $q->where('language_id', $languageId);
                },
                'translations' => function ($q) use ($languageId) {
                    $q->where('language_id', $languageId);
            ...
            ->map(function ($transition) {
                ...
                $toStageName = $transition->toStage ? $transition->toStage->name : null;
                if ($transition->toStage && $transition->toStage->relationLoaded('translations')) {
                    $toStageTranslation = $transition->toStage->translations->first();
                    if ($toStageTranslation && $toStageTranslation->name !== null && $toStageTranslation->name !== '') {
                        $toStageName = $toStageTranslation->name;
                    }
                }

                return [
                    'transition_id' => $transition->id,
                    'label' => ($transitionTranslation && $transitionTranslation->label !== null && $transitionTranslation->label !== '')
                        ? $transitionTranslation->label
                        : $transition->label,
    ...
    private function buildStageStructureWithDetails(Stage $stage, array $existingValues, int $languageId): array
    {
        ...
        // Ensure stage translation is loaded for this language
        if (!$stage->relationLoaded('translations')) {
            $stage->load(['translations' => function ($q) use ($languageId) {
                $q->where('language_id', $languageId);
            }]);
        }

        $stageTranslation = $stage->translations->first();
        $stageName = ($stageTranslation && $stageTranslation->name !== null && $stageTranslation->name !== '')
            ? $stageTranslation->name
            : $stage->name;

        $sections = [];

        foreach ($stage->sections as $section) {
            // Load section translations if not loaded
            if (!$section->relationLoaded('translations')) {
                $section->load(['translations' => function ($q) use ($languageId) {
                    $q->where('language_id', $languageId);
                }]);
            }

            $sectionTranslation = $section->translations->first();
            $sectionName = ($sectionTranslation && $sectionTranslation->name !== null && $sectionTranslation->name !== '')
            ...
            $sections[] = [
                ...
                'section_name' => $sectionName, // LOCALIZED
        ...
        return [
            'stage_id' => $stage->id,
            'stage_name' => $stageName, // LOCALIZED
    ...
    public function getEntryByPublicIdentifier(string $publicIdentifier, ?int $userId = null, ?int $languageId = null): array
    {
        ...
        // Load translations (fields + stages + sections + transitions + toStage translations)
        $entry->load([
            'formVersion.stages.translations' => function ($q) use ($languageId) {
                $q->where('language_id', $languageId);
            },
            'formVersion.stages.sections.translations' => function ($q) use ($languageId) {
                $q->where('language_id', $languageId);
            },
            'formVersion.stages.sections.fields.translations' => function ($q) use ($languageId) {
                $q->where('language_id', $languageId);
            },
            // For transitions list below (we load there too, but safe to keep minimal here)
        ]);
        ...
        // Make sure each stage has translations loaded for language
        $allStages->load(['translations' => function ($q) use ($languageId) {
            $q->where('language_id', $languageId);
        }]);
        ...
        foreach ($allStages as $stage) {
            ...
            if ($isCurrentStage || $isPreviousStage) {
                // Localize stage name for list item (structure also includes localized)
                $stageTranslation = $stage->translations->first();
                $stageName = ($stageTranslation && $stageTranslation->name !== null && $stageTranslation->name !== '')
                    ? $stageTranslation->name
                    : $stage->name;
        ...
        // Get available transitions from current stage (localized label + to stage name)
